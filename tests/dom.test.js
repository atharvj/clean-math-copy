...
test('whole phasor explanations combine structured and token-only source-less CHTML while omitting a hidden diagram transcript', () => {
  ...
  // Static/pre-rendered CHTML often has no TeX metadata or live MathJax
  // object. A plain identifier is still a complete genuine renderer root,
  // and must not cancel the structured roots elsewhere in the selection.
  const outputSymbol = rendered('output-symbol', (token) => token('mi', '𝑉'));
  ...
  const instance = dom([
    ...
    '<p>The overall output voltage vector ', outputSymbol, ' is shown.</p>',
  ...
  assert.equal(payload.text, [
    ...
    'The overall output voltage vector V is shown.',
  ...
  // Exercise the installed event path as well as the pure serializer. If any
  // one root makes the payload fail closed, the browser's native default copy
  // runs after dispatch and reintroduces both flattened math and image alt.
  ...
  assert.doesNotMatch(
    (clipboard.get('text/plain') || '') + (clipboard.get('text/html') || ''),
    /Three sequential|coordinate system|<img\b/iu
  );
});

test('source-less MathJax v4 token roots require an explicit CHTML identity without semantic IDs', () => {
  // Generated by @mathjax/src 4.1.3 for `V`, then stripped only of its
  // data-latex source metadata. Default MathJax v4 CHTML does not emit
  // data-semantic-id attributes, so the renderer topology and fully consumed
  // visible token are the independent evidence for this source-less leaf.
  const officialLeaf = [
    '<mjx-container id="target" class="MathJax" jax="CHTML" overflow="overflow">',
    '<mjx-math class="NCM-N"><mjx-mi><mjx-c class="mjx-c1D449">𝑉</mjx-c>',
    '</mjx-mi></mjx-math></mjx-container>'
  ].join('');
  const copy = (markup, outputMode) => {
    const instance = dom(markup, 'https://unrelated.example/math');
    const target = instance.window.document.querySelector('#target');
    assert.equal(target.querySelector('[data-semantic-id]'), null);
    return cleanCopy.getCopyPayload(
      instance.window.document,
      selectContents(instance.window, target),
      { outputMode },
  ...
  assert.equal(copy(officialLeaf, 'faithful').text, 'V');
  assert.equal(copy(officialLeaf, 'calculator').text, 'V');
  assert.equal(copy(officialLeaf, 'latex').text, '$V$');
  assert.equal(
    copy(officialLeaf.replace(' jax="CHTML"', ''), 'faithful'),
    null,
    'a generic custom-element lookalike without MathJax CHTML identity must fail closed'
  );
});

test('source-less CHTML rejects a visible speech sibling for whole and mixed selections', () => {
  const instance = dom([
    '<p id="target">before <mjx-container id="math" class="MathJax" jax="CHTML">',
    '<mjx-math aria-hidden="true"><mjx-mi><mjx-c>x</mjx-c></mjx-mi></mjx-math>',
    '<mjx-speech>VISIBLE SPEECH</mjx-speech>',
    '</mjx-container> after</p>'
  ].join(''), 'https://unrelated.example/math');
  const target = instance.window.document.querySelector('#target');
  ...
  assert.equal(
    cleanCopy.getCopyPayload(
      instance.window.document,
      selectContents(instance.window, math),
      cleanCopy.DEFAULT_SETTINGS,
      instance.window,
      math
    ),
    null,
    'a whole renderer selection must not discard visible sibling text'
  );
  assert.equal(
    cleanCopy.getCopyPayload(
      instance.window.document,
    ...
    null,
    'a mixed selection must stay native rather than suppress visible sibling text'
  );
});
